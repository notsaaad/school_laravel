    ...
    <div class="content container mt-3">
        ...
        @if (count($carts) == 0)
            <div class="text-center d-flex flex-column  align-items-center" style="font-size: 30px">
                ...
                <div class="empty my-3"> Your shopping cart is empty
                </div>

                <x-form.link class="es-btn-primary" title="Show Items" path="students/items"></x-form.link>
            </div>
        @else
                    ...
                    <tbody>
                        ...
                        @foreach ($carts as $cart)
                            <tr>
                                ...
                                <td><img width="50" class="prodcut_img" src="{{ path($cart->product->img) }}"
                                        alt="{{ $cart->product->name }}"></td>

                                <td>{{ $cart->product->name . ' [ ' . $cart->variant->value . ' ] ' }}
                                </td>
                                ...
                                <td>{{ $cart->product->sell_price * $cart->qnt }} </td>


                                @php
                                    $total += $cart->product->sell_price * $cart->qnt;
                                @endphp




                                <x-td_end normal id="{{ $cart->id }}"
                                    name="{{ $cart->product->name . ' [ ' . $cart->variant->value . ' ] ' }}">
            ...
            <x-form.delete title="Product" path="students/cart/destroy"></x-form.delete>
        @endif


        @if (count($carts) > 0)
            <button onclick="buy_package()" class="bay_btn my-2"> CheckOut <svg width="22"
            ...
            function buy_package() {
                ...
                if (allSelected) {
                    ...
                    <div style="text-align: left;">
                        ...
                        <p><strong>Package Price:</strong> {{ $total }} EGP</p>
                        <p><strong>Service Fee:</strong>     @if (settings('service_expenses') != 0)
                                    {{ number_format((settings('service_expenses') * $total) / 100, 2) }} EGP
                                @else
                                    Free
                                @endif  </p>
                        <p><strong>Total:</strong> {{ number_format((settings('service_expenses') * $total) / 100 + $total, 2) }} EGP </p>
                    </div>
                `,
                        showDenyButton: true,
                        showCancelButton: false,
                        confirmButtonText: "Yes, Buy For  {{ number_format((settings('service_expenses') * $total) / 100 + $total, 2) }}  EGP",
                        icon: "info",
                        denyButtonText: `Cancel`
                    }).then((result) => {
                        if (result.isConfirmed) {
                            window.location.href = "/students/cart/checkOut"
                        }
                    });
